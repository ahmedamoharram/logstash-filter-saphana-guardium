...
public class Parser {
	...
	public static Time getTime(String dateString) {
		if (dateString.matches("\\d+")) {
			...
			// ahmedm2
			// ZonedDateTime date = ZonedDateTime.parse(dateString.replaceAll(" ", "T").concat("Z"), DateTimeFormatter.ISO_DATE_TIME);
			// No need to concat Z, it will already be there by logstash.
			ZonedDateTime date = ZonedDateTime.parse(dateString.replaceAll(" ", "T"), DateTimeFormatter.ISO_DATE_TIME);
...
//		if (data.has(Constants.SOURCE_PROGRAM) && !data.get(Constants.SOURCE_PROGRAM).isJsonNull() && !data.get(Constants.SOURCE_PROGRAM).getAsString().contains("%"))
		if (data.has(Constants.SOURCE_PROGRAM) && !data.get(Constants.SOURCE_PROGRAM).isJsonNull())
		...
		// ahmedm2 The osUser is the Application_username that is sent by SAP HANA
		if (data.has(Constants.OS_USER) && !data.get(Constants.OS_USER).isJsonNull())
			accessor.setOsUser(data.get(Constants.OS_USER).getAsString());
		else
